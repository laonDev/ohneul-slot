...
**Tech Stack:** React 19 · TypeScript 5.7 · Vite 6 · `@apps-in-toss/web-framework` 2.6.x (`granite dev` / `ait build`) · Vitest(순수 로직 테스트)

---

## 사전 참고 (런타임 확인 완료)

- **로컬 브라우저/노드에서 플랫폼 브리지는 동작하지 않는다.** 슬롯 햅틱/저장소/공유/푸시는 **샌드박스 앱**에서만 실제 동작한다. 따라서 `platform/` 어댑터는 로컬/테스트에서 안전하도록 **환경 가드 + 폴백**을 갖는다(`typeof window === 'undefined'` → 노드/테스트, in-memory 사용).
- **확인된 API (전부 `@apps-in-toss/web-framework` 최상위에서 import 가능, 런타임 검증):**
  - `Storage.getItem(key): Promise<string|null>`, `Storage.setItem(key, value): Promise<void>`, `removeItem`, `clearItems`
  - `generateHapticFeedback({ type }): Promise<void>` — `type: 'tickWeak'|'tap'|'tickMedium'|'softMedium'|'basicWeak'|'basicMedium'|'success'|'error'|'wiggle'|'confetti'`
  - `share({ message: string }): Promise<void>`
  - `getTossShareLink(path: string, ogImageUrl?: string): Promise<string>`
  - `requestNotificationAgreement(options): Promise<NotificationAgreementResult>`
  - 위치(7월): `getCurrentLocation`, `startUpdateLocation`
- **초기화**: 별도 `registerApp` 없음. 스캐폴드된 `src/main.tsx`가 `createRoot(...).render(<App/>)` 로 마운트(이미 존재).
- **빌드/실행**: `npm run dev`(= `granite dev`, 포트 5173) · `npm run build`(= `ait build`).
...
작업 폴더(`/Users/kimgyubeom/workspace/toss-challenge`) 안에서 비대화형 실행:
```bash
npx create-ait-app@latest ohneul-slot --inline --pm npm
```
`--inline` = 질문 생략(TDS·예제 모두 미설치). 결과: Vite 6 + React 19 + TS 5.7 + `@apps-in-toss/web-framework` ^2.6.1 프로젝트 생성, `npm install`까지 완료됨.

- [ ] **Step 2: 개발 서버 기동 확인**

```bash
cd ohneul-slot
npm run dev          # = granite dev (포트 5173)
```
Expected: 개발 서버 기동, 콘솔에 로컬 URL/스킴 표시. (로컬 브라우저는 플랫폼 브리지가 없어 일부 기능 미동작 — 샌드박스에서 검증)
...
`src/platform/storage.ts` — 정적 import(런타임 확인됨). 토스 환경(브라우저/웹뷰)에서는 `Storage`를, 노드/테스트(`typeof window === 'undefined'`)에서는 in-memory 폴백을 쓰고, 토스 밖 브라우저에서 호출이 실패하면 try/catch로 폴백.
```ts
...
```
> 테스트는 `isNode` 분기로 in-memory를 타므로 결정적이다. 샌드박스에서는 `Storage`가 영구 저장한다.
...
- [ ] **Step 1: 어댑터 작성 (확인된 API `generateHapticFeedback`)**

`src/platform/haptic.ts` — 내부 의미(`tick`/`success`)를 플랫폼 `HapticFeedbackType`으로 매핑. 토스 밖에서는 웹 Vibration 폴백.
```ts
...
- [ ] **Step 1: 공유 어댑터 작성 (확인된 API `share` + `getTossShareLink`)**

`src/platform/share.ts` — 가능하면 `getTossShareLink('/')`로 토스 딥링크를 만들어 메시지에 포함하고, `share({ message })`로 시트를 띄운다. 토스 밖에서는 Web Share/클립보드 폴백.
```ts
...
`requestNotificationAgreement(options): Promise<NotificationAgreementResult>` 의 `RequestNotificationAgreementOptions`/결과 타입을 `node_modules/@apps-in-toss/web-bridge/dist/index.d.ts`에서 확인해 인자/반환 필드를 맞춘다. 푸시 정기 발송은 **콘솔 "Push & Notification"** 에서 보내므로, 앱은 **수신 동의**만 담당. `granite.config.ts`의 `permissions`에 푸시 관련 권한이 필요하면 추가(콘솔 안내 기준).

- [ ] **Step 2: notify 어댑터 작성 (확인된 API `requestNotificationAgreement`)**

`src/platform/notify.ts`:
```ts
import { requestNotificationAgreement } from '@apps-in-toss/web-framework';

/** 점심 알림 수신 동의 요청. 성공 시 true. (정기 발송은 콘솔/플랫폼에서) */
export async function enableLunchPush(): Promise<boolean> {
  try {
    const result = await requestNotificationAgreement();
    // 결과 타입(NotificationAgreementResult)에 따라 동의 여부 필드를 확인해 반환.
    // 예: return result?.agreed === true;  ← Step 1에서 확정한 필드명으로 교체
    return Boolean(result);
  } catch {
    return false; // 토스 밖/로컬
  }
}
```
> 프레임워크에 로컬 노티 스케줄 API가 있으면 평일 11:30 알림 등록으로 확장. 없으면 콘솔 정기 발송으로 daily trigger를 구현.
