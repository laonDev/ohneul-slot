...
- [ ] **Step 1: 실제 API 시그니처 (확인 완료)**

`requestNotificationAgreement(params): () => void` — Promise가 아니라 **콜백+구독 해제** 패턴.
```ts
...
```
`templateCode`는 **콘솔의 푸시 템플릿 코드**(Task 17에서 발급). 정기 발송은 콘솔 "Push & Notification"에서 한다 — 앱은 **수신 동의**만 담당.

- [ ] **Step 2: notify 어댑터 작성**

`src/platform/notify.ts` — 콜백 API를 Promise<boolean>로 감싼다.
```ts
import { requestNotificationAgreement } from '@apps-in-toss/web-framework';

// 콘솔에서 푸시 템플릿 등록 후 코드로 교체 (Task 17). 비어 있으면 동의 요청 불가.
...
```
> 정기 11:30 발송은 콘솔 푸시 스케줄로 구현(daily trigger).
